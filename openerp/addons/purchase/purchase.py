...
class purchase_order(osv.osv):
    ...
    def _invoiced(self, cursor, user, ids, name, arg, context=None):
        ...
        for purchase in self.browse(cursor, user, ids, context=context):
            invoiced = False
            # HACK: 02.02.2015 16:34:12: jool1: special hack, because if the invoice_rate is 99% or 101% it will not be set to true
            #if purchase.invoiced_rate == 100.00:
            if purchase.invoiced_rate > 0.00:
                invoiced = True
            res[purchase.id] = invoiced
        return res

    #hack jool: added special function to recalculate the invoiced flag -> button in view added manually
    #<button name="recalculate_invoiced_flag" string="SPECIAL REFRESH" type="object" class="oe_highlight"/>
    def recalculate_invoiced_flag(self, cr, uid, ids, context=None):
        po_ids = self.search(cr, uid, [('state', '!=', 'done')], context=context)
        for po in self.browse(cr, uid, po_ids, context=context):
            new_text = ' '
            if po.client_order_ref:
                new_text = po.client_order_ref + ' '
            self.write(cr, uid, [po.id], {'client_order_ref' : new_text})
        return True   
