    ...
    def get_lines(self, data):
        ...
        for report in self.pool.get('account.financial.report').browse(self.cr, self.uid, ids2, context=data['form']['used_context']):
            ...
            if report.type == 'accounts' and report.account_ids:
                ...
                company_id=None
                company_id = self.pool.get('account.account').browse(self.cr,self.uid,data['form']['chart_account_id']).company_id.id
                if company_id:    
                    account_ids = account_obj.search(self.cr, self.uid, [('company_id','=',company_id),('user_type','in', [x.id for x in report.account_type_ids])])
                else:
